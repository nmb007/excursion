...
class UserSearch extends User
{
    ...
    {
        return [
            [['user_name', 'email', 'status', 'item_name'], 'safe'],
        ];
    }
     ...
     */
    public function search($params, $pageSize = 10, $admin = false)
    {
        $query = User::find()->joinWith('role');

        // we make sure that editor- roles can not see users with Admin role
        if ($admin === false) 
        {
            $query->where(['!=', 'item_name', 'admin']);
        }
